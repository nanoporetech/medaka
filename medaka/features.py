...
class SampleGenerator(object):
    ...
    def training_samples(self, max_label_len):
        ...
        for s in self.samples:
            ...
            # s.labels is a structured array with run-length encoded (base,
            # length) labels.
            # the dimension of the last axis determines the ploidy.
            # if the ploidy is greater than 1, "multi-hot-encoding" labels will
            # be created i.e. a heterozygous locus would have >1 non-zero elements
            ...
            for p in range(ploidy):
                labels = s.labels[:, p]
                hap_ys.append(np.fromiter((label_encoding[tuple(l)] for l in labels), dtype=int, count=len(labels)))
