...
from medaka.util.generators import serve_data_batch, serve_sample_batch
from medaka.util.sequences import (get_benchmark, model_output_to_fasta,
                                   get_original_sequence)
...
def train_lstm(data, label, tvt_ratio, data_pp, batch_size, window_size,
    ...
    write_original_and_corrected_sequence(X_test, model, threads=threads,
                                          name='_'.join([out_prefix, 'test_sequence']))
...
def get_parser():
    ...
    parser.add_argument('--threads', type=int, default=1,
                        help='number of concurrent training threads'),
    parser.add_argument('--class_weights', type=str, default=None,
                        help='json-formatted dictionary of class weights')
...
def main():
    ...
    if args.class_weights:
        ...
        args.class_weights = {int(k): float(v)
                              for k, v in args.class_weights.items()}
