...
def reindex_labels(truth, new_positions):
    ...
    # proceeding base to include the missing bases. When this happens,
...
def bams_to_pileup(bams, ref_fasta, ref_name, start, end, truth_bam=None, base_ratios=None):
    ...
    # create a common index for all pileups
    if len(pileups) > 1:
    ...
    if truth_bam is not None:
        ...
        t0 = now()
        labels = reindex_labels(truth, pileups[0].positions)
        t1 = now()
...
def generate_pileup_chunks(bams, ref_fasta, ref_name, start=0, end=None,
    ...
    """Yield overlapping chunks of pileup generated by `bams_to_pileup`.
...
def rechunk(pileup_gen, chunk_size=1000, overlap=200):
    ...
    for pileups, labels in pileup_gen:
        ...
        for pos in pos_chunker:
            logger.debug("Chunking positions {}:{} ({} bases)".format(pos['major'][0], pos['major'][-1], len(pos)))
